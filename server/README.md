...
You should now be able to make changes to `server.js` and our server should be
...
First let's move our server `server.js` file to `lib/server.js`.

```shell
$ mv server.js lib/server.js
```
...
And create our test in `test/server.js`.
